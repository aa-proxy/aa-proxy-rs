        ...
        initial_content_keycode: if profile.display_type == DisplayType::DISPLAY_TYPE_AUXILIARY {
            Some(
                profile
                    ...
                    .unwrap_or(KeyCode::KEYCODE_NAVIGATION),
            )
        } else {
            None
        },
...
fn create_media_sink_service(id: i32, profile: DisplayProfile) -> Service {
    ...
    if let Some(insets) = &profile.content_insets {
        if let Some(v) = insets.top    { content_insets.set_top(v); }
        if let Some(v) = insets.bottom { content_insets.set_bottom(v); }
        if let Some(v) = insets.left   { content_insets.set_left(v); }
        if let Some(v) = insets.right  { content_insets.set_right(v); }
    }
    ...
    if let Some(insets) = &profile.stable_content_insets {
        if let Some(v) = insets.top    { stable_insets.set_top(v); }
        if let Some(v) = insets.bottom { stable_insets.set_bottom(v); }
        if let Some(v) = insets.left   { stable_insets.set_left(v); }
        if let Some(v) = insets.right  { stable_insets.set_right(v); }
    }
...
pub fn add_display_services(
    ...
    for profile in display_profiles(msg, cfg) {
        ...
        msg.services
            .push(create_media_sink_service(media_service_id, profile.clone()));

        let input_service_id =
            if profile.input_source && !has_input_display(msg, profile.display_id) {
                let input_service_id = next_service_id(msg);
                msg.services.push(create_input_source_service(
                    input_service_id,
                    profile.clone(),
                ));
