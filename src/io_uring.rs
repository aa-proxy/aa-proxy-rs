...
pub async fn media_tap_reverse_bridge_once(android_ip: IpAddr, endpoint: MediaTapEndpointInfo) -> io::Result<()> {
    ...
    let mut remote = match timeout(Duration::from_secs(3), TokioTcpStream::connect(&remote_addr)).await {
        Ok(Ok(stream)) => stream,
        Ok(Err(e)) => {
            error!(
    ...
    let mut local = match timeout(Duration::from_secs(3), TokioTcpStream::connect(&local_addr)).await {
        Ok(Ok(stream)) => stream,
        Ok(Err(e)) => {
            error!(
