...
fn serialize_display_type<S>(value: &DisplayType, serializer: S) -> std::result::Result<S::Ok, S::Error>
where
    S: Serializer,
{
...
pub async fn add_display(
    ...
    if file.displays.iter().any(|p| p.id == profile.id) {
        return Err(anyhow!("injected display id already exists: {}", profile.id));
    }
