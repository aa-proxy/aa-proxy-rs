...
#[cfg(feature = "wasm-scripting")]
use aa_proxy_rs::script_wasm::{
    ScriptParameters, ScriptRegistry
};
...
fn main() -> Result<()> {
    ...
    #[cfg(feature = "wasm-scripting")]
    let script_registry = start_wasm_engine(&mut runtime, WASM_HOOKS_DIR.to_string(), script_parameters).ok();
